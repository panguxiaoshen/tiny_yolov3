...
class Dataset(object):
    ...
    def __init__(self, dataset_type):
        ...
        self.anchors = np.array(utils.get_anchors(cfg.YOLO.TINY_ANCHORS, is_tiny=True))
    ...
    def __next__(self):
        ...
        batch_image = np.zeros((self.batch_size, self.train_input_size, self.train_input_size, 1))
    ...
    def Contrast_and_Brightness(self,image, alpha, beta):
        ...
        return dst
    def random_distort_color(self, image, bboxes):
        if random.random() < 0.5:
            alpha = random.random()+0.5
            beta  = random.random()+0.5
            image = self.Contrast_and_Brightness(image,alpha, beta)
        return image, bboxes
    ...
    def parse_annotation(self, annotation):
        ...
        if self.data_aug:
            image, bboxes = self.random_distort_color(np.copy(image), np.copy(bboxes))
