...
from yolo3.model import preprocess_true_boxes, yolo_body, tiny_yolo_body, yolo_loss, yolo_head, yolo_eval
from keras.layers import Input, Lambda

def load_model_from_h5(model_path):
    try:
        yolo_model = load_model(model_path, compile=False)
        print("can't load the model by  --load_model\n")
    except:
        yolo_model = tiny_yolo_body(Input(shape=(None, None, 1)), 3, 1)
...
class WeightSaver(object):
    ...
    def __init__(self, h5_path, output_path):
        self.model = load_model_from_h5(h5_path)
